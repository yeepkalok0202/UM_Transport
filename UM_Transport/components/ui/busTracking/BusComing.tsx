...
const BusComing: React.FC<BusComingProps> = ({
    id, busType, totalTime, fee, busPlate, route
}) => {
    return (
...
const Route: React.FC<RouteProps> = (
    { location, time, arrived, current }
) => {
    return (
        <View style={{paddingVertical: 10}}>
           <View style={{flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between'}}>
            <View style={{flexDirection: 'row', alignItems: 'center'}}>
                <View style={[styles.iconBackground, {backgroundColor: arrived ? '#979797' : '#4285F4',}]}>
                    <Entypo name="triangle-down" size={12} color="white" />
                </View>

                <Text style={[styles.location, {width: 150, color: arrived ? '#979797' : '#4285F4', fontWeight: current ? '700' : '400'}]} numberOfLines={1}>{location}</Text>
            </View>

            <Text style={[styles.location, {fontSize: 10, color: arrived ? '#979797' : '#4285F4', fontWeight: current ? '700' : '400'}]}>
