import { Text, View, StyleSheet, FlatList, TouchableOpacity, Modal, TouchableWithoutFeedback, Image, ScrollView } from "react-native";
      ...
      <ScrollView style={styles.container}>
        <View style={{flexDirection:"row", alignItems:"center", justifyContent:"space-between"}}>
          <View style={{flexDirection:"row", alignItems:"center"}}>
            ...
            <Text style={styles.locationText} numberOfLines={1}> Faculty Computer Science and Technology Information</Text>
          </View>
        ...
        <View style={{marginTop:20}}>
          ...
          <View style={{flexDirection:"row", flexWrap:"wrap", justifyContent:"space-between"}}>
            {transportChoice.map((item, index) => (
              <TouchableOpacity key={index} style={{width:"48%", marginBottom:10}} onPress={item.route}>
                <View style={{width:"100%", height:110, borderRadius:10, backgroundColor:item.cardColor, justifyContent:"center", alignItems:"center"}}>
                    <Text style={{fontSize:18, color:"#002266", fontWeight:"bold", position:"absolute", top:15, zIndex:1, left:10}}>{item.name}</Text>

                  <Image source={item.image} style={{ position: "absolute", borderRadius: 20}} />
                </View>
              </TouchableOpacity>
            ))}
          </View>
          ...
          <View style={styles.modalOverlay}>
            <TouchableWithoutFeedback onPress={(e) => e.stopPropagation()}>
              <View style={styles.modalContent}>
                <Text style={[styles.title, {textAlign: "center"}]}> Choose your transport</Text>
...
const PublicTransport = () => {
  return(
      <View style={
        ...
        <FontAwesome6 name="bus" size={50} color="white" style={{alignSelf: "center"}}/>
        <Text style={{color: "white", fontSize: 20, fontWeight: "bold", marginTop: 10}}>Public Transport</Text>
      </View>
  );
}
...
const RideHailing = () => {
  return(
      <View style={{
        ...
        <FontAwesome name="car" size={50} color="white" style={{alignSelf: "center"}}/>
        <Text style={{color: "white", fontSize: 20, fontWeight: "bold", marginTop: 10}}>Ride-hailing</Text>
      </View>
  );
}
